	...
	{#if rows.length}
		<svg viewBox={`0 0 ${VBX} ${VBY}`} class="chart">
			...
			{#each rows as r, i (i)}
				...
				<text x={xPos(i)} y={PAD_TOP + plotH + 8} class="axis-x" text-anchor="middle">
					{r.stage}
				</text>
			{/each}

			<text x={PAD_LEFT + plotW / 2} y={PAD_TOP - 6} class="title" text-anchor="middle"
				>Эффективность открытий (flips/cards)</text
			>
			<text x={PAD_LEFT + plotW / 2} y={VBY - 2} class="title" text-anchor="middle">Этап</text
			>
		</svg>
