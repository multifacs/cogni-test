...
export function estimateDifficulty(
	...
	const visibleElements = task.cells.reduce(
		(sum, cell, index) => (index === task.missingIndex ? sum : sum + cell.elements.length),
		0
	);
	...
	const ruleComplexity = Math.min(
		6,
		task.rules.length + (hasSetLogic ? 2 : 0) + (colorPolicy === 'rule' ? 1 : 0)
	);
	const objectComplexity =
		task.componentModel === 'regional_overlay' || task.componentModel === 'regional_texture'
			...
			: task.componentModel === 'grid_bitmask' || task.componentModel === 'radial_components'
				? 2
				: 1;
	const attributeComplexity = task.rules.some((r) =>
		['set', 'regional', 'relation'].includes(r.attributeType)
	)
		? 3
		: task.rules.length > 1
			? 2
			: 1;
	const topologyComplexity =
		task.sceneTopology === 'grid_3x3' ? 3 : task.sceneTopology === 'grid_2x2' ? 2 : 1;
	...
	const layerAmbiguity =
		hasWrongLayer || hasNestedObjects || task.componentModel === 'regional_overlay' ? 2 : 1;
	const workingMemoryLoad = Math.min(3, Math.ceil((compositionDepth + visibleElements / 16) / 2));
	const perceptualLoad =
		task.componentModel === 'regional_texture' || task.componentModel === 'regional_overlay'
			? 3
			: visualDensity;
