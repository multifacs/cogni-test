<script lang="ts">
	...
	function shapePoints(shape: ShapeKind, cx: number, cy: number, size: number): string {
		const r = size / 2;
		if (shape === 'triangle')
			return `${cx},${cy - r} ${cx - r},${cy + r * 0.78} ${cx + r},${cy + r * 0.78}`;
		if (shape === 'diamond')
...
<div class:compact class="cell-shell">
	<svg viewBox="0 0 100 100" aria-hidden="true" class="cell-svg">
		...
		{#if missing}
			<text x="50" y="61" text-anchor="middle" font-size="34" font-weight="800" fill="#8b94a3"
				>?</text
			>
		{:else if cell}
			{#each cell.elements as el (el.id)}
				...
				{:else if el.type === 'petal'}
					<g transform="rotate({el.angle} 50 50)">
						{#if el.variant === 'bar' || el.variant === 'triangle'}
							<path
								d={petalPath(el.variant)}
								fill={el.fill ?? '#fffdf8'}
								stroke={el.stroke ?? '#172033'}
								stroke-width={el.strokeWidth ?? 3.4}
				...
				{:else if isShapeElement(el)}
					<g
						transform="rotate({el.rotation ?? 0} {el.cx} {el.cy})"
					...
					>
						...
						{:else}
							<polygon
								points={shapePoints(el.shape, el.cx, el.cy, el.size)}
								fill={shapeFill(el)}
								stroke={el.stroke ?? '#172033'}
								stroke-width={el.strokeWidth ?? 4}
								stroke-linejoin="round"
