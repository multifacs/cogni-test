<script lang="ts">
	...
	import { formatMs, summary, type NumbersTrialRow } from './results-adapter';
	...
	let { trials }: { trials: NumbersTrialRow[] } = $props();

	const pointColor = (ctx: ScriptableContext<'line'>) => {
		const result = ctx.raw as NumbersResult | undefined;
	...
	function getResults(trials: NumbersTrialRow[]): NumbersResult[] {
			...
			options: {
				...
				plugins: {
					tooltip: {
						callbacks: {
							title(context) {
								const value = context[0].raw as NumbersResult;
								return `Уровень ${value.x} (${value.mode}, ${value.digitCount} цифр)`;
							},
							beforeBody(context) {
								const value = context[0].raw as NumbersResult;
								const seq = JSON.parse(value.sequence);
								const sub = JSON.parse(value.submitted);
								return [`Ожидалось: [${seq}] → Ответ: [${sub}]`];
							},
							label(context) {
								const value = context.raw as NumbersResult;
