<script lang="ts">
	...
	let { category, adjectiveChoices, setAdjective, shapeChoices, originalColor } = $props();
</script>

{#if category === 'words'}
	<h2 class="text-center">Выберите новое прилагательное:</h2>
	<div class="grid max-w-lg grid-cols-2 gap-4">
		{#each adjectiveChoices as adj (adj)}
	...
	<div class="grid grid-cols-3 gap-4">
		{#each shapeChoices as shape (shape)}
			...
			>
				...
				<span class="mt-2 text-sm">{shape.name}</span>
			</button>
		{/each}
	</div>
{/if}
