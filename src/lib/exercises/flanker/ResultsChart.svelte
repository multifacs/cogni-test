<script lang="ts">
	...
	import { formatMs, summary, type FlankerTrialRow } from './results-adapter';
	...
	let { trials }: { trials: FlankerTrialRow[] } = $props();

	const pointColor = (ctx: ScriptableContext<'line'>) => {
		const result = ctx.raw as FlankerResult | undefined;
	...
	function getResults(trials: FlankerTrialRow[]): FlankerResult[] {
			...
			options: {
				...
				plugins: {
					tooltip: {
						callbacks: {
							title(context) {
								const value = context[0].raw as FlankerResult;
								return `Проба ${value.x}`;
							},
							afterTitle(context) {
								const value = context[0].raw as FlankerResult;
								return `${value.target} → ${value.selected}`;
							},
							beforeBody(context) {
								const value = context[0].raw as FlankerResult;
								return [
									`${value.congruent ? 'Сонаправленный' : 'Несонаправленный'}`
								];
							},
							label(context) {
								const value = context.raw as FlankerResult;
