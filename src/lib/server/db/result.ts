...
export async function getFirstResult<T extends keyof TestResultMap>(
    testType: T,
    userId: string
    ...
    try {
        ...
        // Находим результат с минимальным createdAt
        return data.reduce((earliest, curr) =>
            new Date(curr.createdAt) < new Date(earliest.createdAt) ? curr : earliest
        );
    } catch (error) {
        console.error(`Ошибка при получении первого результата по ${testType}:`, error);
        return null;
    }
}
