...
| `--main-accent-color`    | `#1f3d7a`                                                                                                                                                                      | **Синий из кнопочной пары** (`--button-blue-text`): focus-ring инпутов, `accent-color` радио/чекбоксов, hover-ссылок, акцентные линии карточек. |
| `--input-bg-color`       | `#e5e7eb`                                                                                                                                                                      | Фон текстовых инпутов (общий для `TextInput`/`DateInput`).             |
| `--error-color`          | `#b5544b`                                                                                                                                                                      | **Приглушённый красный** для текстов и рамок ошибок (валидация, сервер). Контраст на белом ≈ 4.9:1. |
...
### 4.5 Паттерн ошибок валидации: анимированный слот (grid-template-rows)

Файлы: `src/lib/components/ui/TextInput.svelte`, `DateInput.svelte`.

Ошибки не появляются скачком — слот раскрывается/сворачивается плавно:

```svelte
<div class="error-slot" class:open={!!errorMessage}>
  <p id={errorId} class="error-text" aria-live="polite">{lastError}</p>
</div>
...
<style>
  ...
  .error-slot.open { grid-template-rows: 1fr; }
  .error-slot > p { overflow: hidden; min-height: 0; }
  @media (prefers-reduced-motion: reduce) {
    .error-slot { transition: none; }
  }
</style>
```

Ключевые детали:
- **`lastError`** — слот помнит последний текст ошибки: при закрытии контент схлопывается вместе с текстом (аккордеон), иначе Svelte опустошает `<p>` и снимает класс в одном кадре, и закрытие становится мгновенным. Сброс ошибки — в пустую строку `''`, поэтому фолбэк через `||`, не `??`.
- `<p>` живёт в DOM всегда; `id` для `aria-describedby` вешается **только при активной ошибке** — скринридер не видит пустых сущностей.
- **Валидация — на `blur`, сброс ошибки — на `input`.** Не валидируйте по мере набора — рамка краснеет на первом символе.
- Цвет — `var(--error-color)`, для рамки — `border-[var(--error-color)]`.
- Лейблы привязаны к инпутам через генерируемый `id` (`$derived(id ?? name)`).
...
| Роль              | Значение                                  | Где                                         |
| ----------------- | ----------------------------------------- | ------------------------------------------- |
| Бренд / акцент    | `#1f3d7a` (`--main-accent-color`)         | Focus-ring инпутов, `accent-color` радио/чекбоксов, hover-ссылки, акцентные линии карточек. |
| Ошибка            | `#b5544b` (`--error-color`)               | Тексты и рамки ошибок валидации/сервера. НЕ для data-viz (красные в играх/графиках — отдельная семантика). |
...
- **Акцент ≠ аватар.** `--avatar-bg-color: #d48c7a` — отдельный токен-фолбэк аватарок (с парным `--avatar-text-color: #4a241a`), он не меняется вместе с `--main-accent-color`.
- **Тёплый терракотовый `#d48c7a`** остаётся в системе только как: фолбэк аватара и тёплый blob в градиенте `--app-bg` — это фон, не акцент.

### 6.1 Ловушка Tailwind-слоёв (важно)

Tailwind v4 кладёт утилиты в `@layer utilities`. **Любое неслоёное CSS-правило в `app.css` перебивает любой Tailwind-класс** (неслоёные всегда сильнее слоёных). Исторически правило `html, body, p, h1… label { color: … }` вне слоёв молча глушило `text-red-600`, `text-white` и т.п. — `color` из base-селектора убран, цвет наследуется от `body`.

**Правило:** при добавлении глобальных стилей в `app.css` либо кладите их в `@layer base`, либо не задавайте в них свойства, которые Tailwind-утилиты могут перекрывать (`color`, `background`, `font-weight`…).
...
- [ ] Ошибки валидации: цвет `var(--error-color)`, анимированный слот (§4.5), валидация на `blur` / сброс на `input`, лейбл привязан через `id` к инпуту.
